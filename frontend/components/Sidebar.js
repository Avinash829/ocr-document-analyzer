...
export default function Sidebar({ isOpen, setIsOpen, isCollapsed, setIsCollapsed }) {
  ...
  const renderNavItem = (item) => (
    <a
      ...
      className={`flex items-center rounded-xl transition-all ${isCollapsed ? "justify-center h-12 w-12 mx-auto" : "gap-3 px-4 py-3"
        } ${item.active
  ...
  return (
    <>
      ...
      {/* Sidebar Container */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex flex-col bg-white transition-all duration-300 ease-in-out lg:static lg:translate-x-0 lg:rounded-2xl lg:shadow-xl border-r lg:border-none border-slate-200 ${isOpen ? "translate-x-0" : "-translate-x-full"
      ...
      >
        ...
        <div className={`mt-4 ${isCollapsed ? "px-4" : "px-5"}`}>
          <button className={`flex items-center justify-center gap-2 rounded-full bg-[#303030] text-sm font-medium text-white shadow-[0_0_0_2px_#EA643A] hover:bg-[#404040] transition-all overflow-hidden ${isCollapsed ? "h-12 w-12 p-0 mx-auto" : "w-full px-4 py-3"
            }`}>
